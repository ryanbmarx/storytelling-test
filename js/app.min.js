...
"use strict";function getCurrentStory(){return Number(location.href.split("?")[1])}function addStoryMeta(e){document.querySelector("#headline").innerHTML=e.headline,document.querySelector("#image img").setAttribute("src",e.image)}function addRefer(e,t){console.log("target is",e,t);var r=document.querySelector("#"+e);r.style.backgroundImage="url("+t.image+")",r.innerHTML="<div class='refer__text'>\n\t\t\t<small class='refer__kicker'>"+kickers[e]+"</small>\n\t\t\t<h2 class='refer__headline'>\n\t\t\t\t<a id='refer-"+e+"-headline' href='/"+t.url+"' class='refer__link'>"+t.headline+"</a>\n\t\t\t</h2>\n\t\t\t<small class='refer__readtime'>"+t.readtime.replace("min","minute")+" read</small>\n\t\t</div> \n\t\t<a class='refer__icon' aria-hidden='true' aria-describedby='refer-"+e+"-headline' href='/"+t.url+"'>\n\t\t\t<img class='' src='img/arrow.svg' alt=\"\" />\n\t\t</a>"}function addRefers(e,t){console.log("current story is",t);var r={previous:1===t?e.length-1:t-2,next:t>=e.length?0:t};Object.keys(r).forEach(function(t){console.log(t,r,r[t]),addRefer(t,e[r[t]])})}require("whatwg-fetch");var kickers={previous:"Previous reading",next:"Keep reading"};window.addEventListener("DOMContentLoaded",function(e){console.log("DOMContentLoaded"),fetch("https://www.gannett-cdn.com/labs/sdt/developer-mock-nav.json").then(function(e){return e.json()}).then(function(e){addStoryMeta(e.items[getCurrentStory()-1]),addRefers(e.items,getCurrentStory())}).then(function(){document.querySelector("#refers").style.opacity=1}).catch(function(e){console.log("parsing failed",e)})});
